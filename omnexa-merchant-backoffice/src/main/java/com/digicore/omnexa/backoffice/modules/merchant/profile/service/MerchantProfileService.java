 ...
 *
 * <p>This service interacts with the {@link MerchantFeignClient} to fetch and process
 * merchant profile data. It provides methods to retrieve all profiles, search profiles
 * by a term, and filter profiles by status, all with pagination support.
 *
 * <p>Implements the {@link ProfileService} interface.
 * Uses {@link RequestUtil} for response data conversion.
 *
 * <p>Logs errors and operations using SLF4J {@link lombok.extern.slf4j.Slf4j}.
 * Handles exceptions gracefully and transforms raw response data into a paginated format.
 *
 * <p>Author: Onyekachi Ejemba
 * Created On: Aug-04(Mon)-2025
 */
@Slf4j
@Service
...
public class MerchantProfileService implements ProfileService {
     ...
     *
     * <p>This method handles the Feign client call, processes the response, and converts
     * it into a paginated response format. Logs any errors encountered during execution.
     *
    ...
    private PaginatedResponse<ProfileInfoResponse> executeProfileRequest(
            Supplier<ResponseEntity<ApiResponseJson<Object>>> feignCall,
     ...
     *
     * <p>This method uses {@link RequestUtil} to map the raw response data into a
     * {@link PaginatedResponse} of {@link MerchantProfileInfoResponse}, and then
     * transforms it into a {@link PaginatedResponse} of {@link ProfileInfoResponse}.
     *
     * @param responseData the raw response data to convert
     * @return a paginated response containing profile information
     */
    private PaginatedResponse<ProfileInfoResponse> convertToPaginatedProfileResponse(Object responseData) {
        PaginatedResponse<MerchantProfileInfoResponse> merchantProfileInfoResponse =
                RequestUtil.getObjectMapper().convertValue(responseData,
                        new TypeReference<PaginatedResponse<MerchantProfileInfoResponse>>() {});

        // Convert to ProfileInfoResponse list
        List<ProfileInfoResponse> profileInfoResponses = merchantProfileInfoResponse.getContent()
                .stream()
