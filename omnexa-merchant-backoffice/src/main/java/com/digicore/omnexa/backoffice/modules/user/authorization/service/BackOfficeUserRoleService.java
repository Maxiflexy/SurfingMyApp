...
public class BackOfficeUserRoleService implements RoleService {
  ...
  public void editRole(AuthorizationRequest request) {
    ...
    // Delegate validation to helper
    authorizationHelper.validateRoleNameForUpdate(roleCreationDTO.getName());
    authorizationHelper.validateRolePermissions(roleCreationDTO.getPermissions());
    ...
    // MERGE PERMISSIONS: Only if new permissions are provided
    if (roleCreationDTO.getPermissions() != null && !roleCreationDTO.getPermissions().isEmpty()) {
      // Get existing permission names
      Set<String> existingPermissionNames = existingRole.getPermissions()
              .stream()
              .map(BackOfficeUserPermission::getName)
              .collect(Collectors.toSet());

      existingPermissionNames.addAll(roleCreationDTO.getPermissions());

      // Update permissions using existing helper
      existingRole.setPermissions(
              authorizationHelper.retrieveSelectedPermissions(existingPermissionNames));
    }
