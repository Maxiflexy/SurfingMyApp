...
public class BackOfficeUserProfileService implements ProfileService {
    ...
    public OnboardingResponse createProfile(OnboardingRequest profile) {
        ...
        throw new OmnexaException(
                messagePropertyConfig.getOnboardMessage(INVALID, SYSTEM_DEFAULT_INVALID_REQUEST_ERROR), HttpStatus.BAD_REQUEST);
    }
    ...
    public PaginatedResponse<ProfileInfoResponse> searchProfilesPaginated(
        ...
        Pageable pageable = RequestUtil.createPageable(validatedPageNumber, validatedPageSize, "createdDate");
    ...
    public PaginatedResponse<ProfileInfoResponse> getProfilesByStatusPaginated(
        ...
        log.debug("Retrieving profiles with status='{}', page={}, size={}", profileStatus, pageNumber, pageSize);
        ...
        Page<BackOfficeUserProfileDTO> userPage =
                backOfficeUserProfileRepository.findUsersByStatusPaginated(statusFilter, pageable);
        ...
        requiredFields.forEach(
                (label, supplier) -> {
                    if (RequestUtil.nullOrEmpty(supplier.get())) {
                        String message =
                                messagePropertyConfig.getOnboardMessage(REQUIRED, SYSTEM_DEFAULT_REQUIRED_FIELD_MISSING_ERROR).replace(PROFILE, label);
    ...
    private String buildNotFoundErrorMessage(String profileId) {
        String message = messagePropertyConfig.getOnboardMessage(NOT_FOUND, SYSTEM_DEFAULT_NOT_FOUND_ERROR).replace(PROFILE, profileId);
        log.error(message);
        return message;
    }
    ...
    private String buildDuplicateErrorMessage(String email) {
        String message = messagePropertyConfig.getOnboardMessage(DUPLICATE, SYSTEM_DEFAULT_DUPLICATE_ERROR).replace(PROFILE, email);
        log.error(message);
        return message;
    }
