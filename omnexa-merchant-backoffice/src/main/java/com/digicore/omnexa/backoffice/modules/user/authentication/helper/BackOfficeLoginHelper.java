 ...
 *
 * <p>This helper handles the business logic for creating login responses, managing user
 * permissions, and building JWT tokens for authenticated users.
 *
...
public class BackOfficeLoginHelper {
  ...
  public LoginResponseDTO getLoginResponse(
    ...
    Map<String, String> claims =
        BackOfficeJwtUtil.buildClaims(loginRequestDTO.getUsername(), userDetails);
  ...
  private Set<SimpleGrantedAuthority> getGrantedAuthorities(String assignedRole) {
    ...
    List<String> rolePermissions =
        roleDTO.getPermissions().stream().map(PermissionDTO::getName).toList();
